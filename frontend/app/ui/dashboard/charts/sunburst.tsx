...
const SunburstChart = (props: { data: any, width:number, height:number , show_labels: boolean})  => { // eslint-disable-line
    ...
    const drawChart = () => {
        ...
        const hierarchy = d3.hierarchy(data)
            .sum(d => d.value)
            .sort((a, b) => b?.value ?? 0  - a.?value ?? 0);// eslint-disable-line
        ...
        const arc = d3.arc()
            ...
            .padAngle(d => Math.min((d.?x1 ?? 0 - d.?x0 ?? 0) / 2, 0.005))// eslint-disable-line
            .padRadius(radius * 1.5)
            .innerRadius(d => d.?y0 ?? 0 * radius)// eslint-disable-line
            .outerRadius(d => Math.max(d.?y0 ?? 0 * radius, d.?y1 ?? 0 * radius - 1))// eslint-disable-line
